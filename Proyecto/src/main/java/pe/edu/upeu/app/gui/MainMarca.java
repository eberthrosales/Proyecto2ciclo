...
import pe.edu.upeu.app.dao.MarcaDAO;
import pe.edu.upeu.app.dao.MarcaDaoI;
...
public class MainMarca extends javax.swing.JFrame {
    ...
    public void ListarMarca() {
        mDao = new MarcaDAO();
        List<MarcaTO> listarMarca = mDao.listarMarca();
        ...
        for (int i = 0; i < listarMarca.size(); i++) {
            ob[0] = i+1;
            ob[1] = listarMarca.get(i).getIdMarca();
            ob[2] = listarMarca.get(i).getNombre();
            modelo.addRow(ob);
        }
    ...
    private void paintForm() {
        if (jTable1.getSelectedRow() != -1) {
            ...
            mDao = new MarcaDAO();
            MarcaTO u = mDao.buscarMarca(Integer.parseInt(valor.toString()));
            txtIdUsuario.setText(String.valueOf(u.getIdMarca()));
    ...
    private void btnRegistrarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRegistrarActionPerformed
        ...
        if (fila != -1) {
            //Modificar
            to.setIdMarca(Integer.parseInt(txtIdUsuario.getText()));
            try {
                Validator validator = new Validator(vals);
                if (validator.isPasses()) {
                    int resultado = mDao.update(to);
                    if (resultado != 0) {
                        resetForm();
                        ListarMarca();
